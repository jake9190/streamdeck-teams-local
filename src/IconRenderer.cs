...
public static class IconRenderer
{
    ...
    /// <summary>Build the data URI for an action given the current state.</summary>
    ...
    {
        // Disabled look when there's no meeting to act on.
        bool disabled = !s.MeetingActive && d.Kind != ActionKind.Leave
            || (!s.TeamsRunning);
        if (d.Kind == ActionKind.Leave)
            disabled = !s.MeetingActive || !s.TeamsRunning;

        string bg, fg, glyph;
        bool handBadge = false;

        if (disabled)
        {
            bg = BgDisabled;
            fg = FgDisabled;
            glyph = IdleGlyph(d.Kind);
        }
        else
        {
            switch (d.Kind)
            {
                case ActionKind.ToggleMute:
                    bg = s.Muted ? BgRed : BgNeutral;
                    fg = s.Muted ? FgRed : FgWhite;
                    glyph = s.Muted ? Paths.MicrophoneSlash : Paths.MicrophoneFill;
                    break;
                case ActionKind.ToggleCamera:
                    bg = s.CameraOff ? BgRed : BgNeutral;
                    fg = s.CameraOff ? FgRed : FgWhite;
                    glyph = s.CameraOff ? Paths.VideoCameraSlash : Paths.VideoCameraFill;
                    break;
                case ActionKind.RaiseHand:
                    bg = s.HandRaised ? BgAmber : BgNeutral;
                    fg = s.HandRaised ? FgAmber : FgWhite;
                    glyph = s.HandRaised ? Paths.HandFill : Paths.HandOutline;
                    handBadge = s.HandRaised;
                    break;
                case ActionKind.ShareScreen:
                    bg = s.Sharing ? BgGreen : BgNeutral;
                    fg = s.Sharing ? FgGreen : FgWhite;
                    glyph = s.Sharing ? Paths.MonitorFill : Paths.MonitorOutline;
                    break;
                case ActionKind.Leave:
                    bg = BgLeave;
                    fg = FgWhite;
                    glyph = Paths.PhoneDisconnect;
                    break;
                case ActionKind.Reaction:
                    bg = BgNeutral;
                    fg = ReactionColor(d.Id);
                    glyph = ReactionGlyph(d.Id);
                    break;
                default:
                    bg = BgNeutral; fg = FgWhite; glyph = Paths.MonitorOutline;
                    break;
            }
        }
        ...
        sb.Append("<rect x=\"6\" y=\"6\" width=\"132\" height=\"132\" rx=\"28\" fill=\"").Append(bg).Append("\"/>");
        sb.Append("<g transform=\"translate(31,31) scale(0.32)\" fill=\"").Append(fg).Append("\">");
        sb.Append(glyph);
        sb.Append("</g>");
        if (handBadge)
            sb.Append("<circle cx=\"112\" cy=\"32\" r=\"9\" fill=\"#FFC53D\" stroke=\"").Append(bg).Append("\" stroke-width=\"3\"/>");
        sb.Append("</svg>");
        return sb.ToString();
    }
    ...
    private static string ReactionGlyph(string id) => id switch
    {
        ...
        "react-surprised" => Paths.SmileyXEyesFill,
        _ => Paths.SmileyFill,
    };
